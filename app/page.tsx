...
const linkClass = "underline hover:text-black dark:hover:text-white";

export default async function Page() {
  const [reactHtml, htmlHtml, cssHtml] = await Promise.all([
    highlightToHtml(jsxSnippet, "tsx"),
    highlightToHtml(htmlSnippet, "html"),
    highlightToHtml(cssSnippet, "css"),
  ]);
  ...
  return (
    <main className="relative z-[1] mx-auto w-full max-w-[1200px] px-6 pb-24 pt-8">
      <BlurHaloSurface
        ...
        strength={0.25}
        borderRadius="9999px"
        className="sticky top-4 z-50 mb-[72px]"
      >
        <Nav />
      </BlurHaloSurface>
      ...
      <section
        id="demo"
        className="mb-0 grid grid-cols-1 items-start gap-16 lg:grid-cols-[1fr_1.1fr]"
      >
        <div>
          ...
          <p className="mt-5 max-w-[560px] text-[17px] leading-relaxed tracking-tight text-black/60 dark:text-white/55">
            ...
            , BlurHalo only blurs the area immediately around the dialog. The rest of the page stays
            crisp and readable while the focused surface gets a soft, feathered backdrop blur that
            fades naturally outward.
          </p>
      ...
      <RevealSection>
        <section id="api" className="mt-24">
          ...
          <p className="mb-7 max-w-[680px] text-[15px] leading-relaxed text-black/60 dark:text-white/55">
            BlurHalo renders an independently-sized halo layer behind the dialog using negative
            inset. The halo extends beyond the content and uses a feathered mask so the blur fades
            outward. The dialog body sits above the halo so its content stays sharp.
          </p>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {[
              ...
              >
                ...
                <p className="text-[13px] leading-relaxed text-black/60 dark:text-white/55">
                  {card.desc}
                </p>
              </div>
            ))}
      ...
      <RevealSection>
        <section id="code" className="mt-24">
          ...
          <p className="mb-7 max-w-[680px] text-[15px] leading-relaxed text-black/60 dark:text-white/55">
            Two ways to use BlurHalo — the React component (one file, full interactivity) or plain
            HTML and CSS (zero dependencies, copy and paste).
          </p>
          ...
          <CodeTabs
            tabs={[
              ...
              },
              {
                ...
                html: htmlHtml,
                raw: htmlSnippet,
                note: <>Pair this markup with the CSS tab — no build step, no JavaScript.</>,
              },
              {
                label: "CSS",
                lang: "css",
